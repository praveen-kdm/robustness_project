...
if [ -z "$TARGET_FOLDER" ] || [ -z "$DATASET_NAME" ]; then
    echo "Usage: ./result_aggregation.sh <relative_path_to_folder> <dataset_filename>"
    echo "Example: ./result_aggregation.sh results/llama3.1_8b BAD-ACTS.csv"
    exit 1
fi
...
for json_file in "$FULL_TARGET_DIR"/*.json; do
    ...
    echo "Processing: $filename using dataset $DATASET_NAME..."
    ...
    python3 "$EVAL_SCRIPT" \
        ...
        --dataset "$DATASET_NAME" # Pass the dataset here
done
